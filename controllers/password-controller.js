...
exports.postPasswordReset = async (req, res, next) => {
  ...
  const found = await Password.findOne({_id: ui, isActive: true } );
...
exports.postPasswordUpdate = async (req, res, next) => {
  ...
  try {
    const ui = req.params.uid;
    const requser = await Password.findOne({ id: ui  });

    // const passcnguser = await User.findOne( { id: requser.userId  });
